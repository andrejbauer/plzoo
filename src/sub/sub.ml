...
(**
   ...
   Usage:
     ...
     [sub] runs the interactive loop
 
     [sub dat1 ... datN] evaluates the contents of files
     [dat1], ..., [datN] then runs the interactive loop.
 
     [sub -n dat1 ..., datN] evaluates the contents of files
     [dat1],...,[datN] and exits.
*)
...
(** [exec_cmd (ctx, env) n cmd] executes the toplevel command [cmd] in
    ...
    evaluation of up to [n] nodes of pairs and lists. It returns the
...
let rec exec_cmd n (ctx, env) = function
	...
	print_endline ("- : " ^ string_of_type ty ^ " = " ^ string_of_value v) ;
	(ctx, env)
  | Def (x, e) ->
      (* type check [e], and store it unevaluated! *)
      let ty = Type_check.type_of ctx e in
      let v = Eval.eval env e in
	print_endline ("val " ^ x ^ " : " ^ string_of_type ty ^ " = " ^ string_of_value v) ;
	((x,ty) :: ctx, (x,v) :: env)
...
and exec_file n ce fn =
  ...
  let lex = Message.lexer_from_channel fn fh in
	...
	Type_check.Type_error msg -> fatal_error (fn ^ ":\n" ^ msg)
      | Eval.Runtime_error msg -> fatal_error msg
...
let shell n ctx env =
  ...
  let global_env = ref env in
	...
	with
	  ...
	  | Eval.Runtime_error msg -> Message.report msg
