...
(**
   ...
   Usage:
     ...
     [poly] runs the interactive loop
 
     [poly dat1 ... datN] evaluates the contents of files
     [dat1], ..., [datN] then runs the interactive loop.
 
     [poly -n dat1 ..., datN] evaluates the contents of files
     [dat1],...,[datN] and exits.
*)
...
and exec_file n ce fn =
  ...
  let lex = Message.lexer_from_channel fn fh in
	...
	exec_cmds n ce cmds
    with
	Type_infer.Type_error msg -> fatal_error (fn ^ ":\n" ^ msg)
...
let shell n ctx env =
  ...
  let global_env = ref env in
	...
	with
	  ...
	  | Type_infer.Type_error msg -> Message.report msg
