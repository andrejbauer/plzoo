...
let toplevel_cmd (b:Seq2app.state) (cmd: Presyntax.toplevel_cmd): Syntax.toplevel_cmd  =
  match cmd with
  | Presyntax.Expr e -> Syntax.Expr (Seq2app.parse_presyntax b e)
  | Presyntax.Def (name,  e) -> Syntax.Def (name, Seq2app.parse_presyntax b e)
  | Presyntax.Mixfix (fx, prec, name) -> Syntax.Mixfix (create_operator fx prec name)
  | Presyntax.Quit -> Syntax.Quit

let file b = List.map (fun x -> toplevel_cmd b x)
