...
(** Usage:
    ...
    [minimlerror] runs the interactive loop.

    [minimlerror dat1 ... datN] evaluates the contents of files
    [dat1],...,[datN] then runs the interactive loop.

    [minimlerror -n dat1 ..., datN] evaluates the contents of files
    [dat1],...,[datN] and exits.
*)
...
let shell ctx env =
  print_string ("MiniML+error. Press ") ;
